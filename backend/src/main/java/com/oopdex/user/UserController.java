...
// Class definition for UserController which handles all user profile API
// requests
public class UserController {
    ...
    public ResponseEntity<User> getCurrentUser() {
        ...
        // Look up the user by their email and return a 200 OK response with the User
        // object
    ...
    // Restrict this endpoint to authenticated users with either USER or ADMIN role
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    // Method to search for users by a query string, returning a list of matching
    // users
    public ResponseEntity<List<User>> searchUsers(@RequestParam String query) {
        // Delegate the search to the UserService and return a 200 OK response with the
        // results
    ...
    public ResponseEntity<User> getPublicProfile(@PathVariable Long userId) {
        // Delegate the lookup to the UserService and return a 200 OK response with the
        // found user
